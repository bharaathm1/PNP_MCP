"""
Standalone: df_processlifetime
================================
Extracts process lifetime data using trace.get_processes().
Shows process start/end times, PIDs, image names, and lifetimes within the trace.

PKL: <etl_basename>_df_processlifetime.pkl  (same folder as ETL)
Cache: skips re-analysis if PKL already exists.

Usage:
    speed.exe run standalone_df_processlifetime.py --etl_file <path>

PKL keys:
    df_processlifetime — per-process intervals (columns depend on SPEED version)
"""
...
def extract(trace):
    try:
        if not hasattr(trace, "get_processes"):
            print("[WARNING] get_processes not available"); return pd.DataFrame()
        ...
        print(f"[df_processlifetime] {df.shape}")
        return df
    except Exception as e:
        print(f"[WARNING] extract error: {e}"); return pd.DataFrame()

def main():
    ap = argparse.ArgumentParser(description="Standalone df_processlifetime (speed.exe)")
    ...
    results = {
        "df_processlifetime": extract(trace),
