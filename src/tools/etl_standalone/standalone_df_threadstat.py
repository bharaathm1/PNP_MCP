"""
Standalone: df_threadstat
===========================
Extracts per-thread interval data using trace.get_thread_intervals().
Shows thread CPU usage patterns, scheduling intervals, and per-thread statistics.

PKL: <etl_basename>_df_threadstat.pkl  (same folder as ETL)
Cache: skips re-analysis if PKL already exists.

Usage:
    speed.exe run standalone_df_threadstat.py --etl_file <path>

PKL keys:
    df_threadstat — per-thread interval data (columns depend on SPEED version)
"""
...
def extract(trace):
    try:
        if not hasattr(trace, "get_thread_intervals"):
            print("[WARNING] get_thread_intervals not available"); return pd.DataFrame()
        ...
        print(f"[df_threadstat] {df.shape}")
        return df
    except Exception as e:
        print(f"[WARNING] extract error: {e}"); return pd.DataFrame()

def main():
    ap = argparse.ArgumentParser(description="Standalone df_threadstat (speed.exe)")
