"""
Standalone: df_containmentunpark
==================================
Extracts WpsContainmentUnparkCount events capturing before/after efficient
and performance core unpark counts per containment interval.

PKL: <etl_basename>_df_containmentunpark.pkl  (same folder as ETL)
Cache: skips re-analysis if PKL already exists.

Usage:
    speed.exe run standalone_df_containmentunpark.py --etl_file <path>

PKL keys:
    df_containmentunpark — columns: timestamp, ContainmentEnabled,
        ContainmentCrossOverRequired, BeforeEfficientUnparkCount,
        AfterEfficientUnparkCount, BeforePerfUnparkCount,
        AfterPerfUnparkCount, RawTargetUnparkCount
"""
...
PKL_SUFFIX = "df_containmentunpark"
...
def extract(trace):
    ...
    print(f"[df_containmentunpark] {len(df)} records")
    return df

def main():
    ap = argparse.ArgumentParser(description="Standalone df_containmentunpark (speed.exe)")
    ...
    results = {
        "df_containmentunpark": extract(trace),
