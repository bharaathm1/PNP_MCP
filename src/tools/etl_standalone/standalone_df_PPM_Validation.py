"""
Standalone: df_PPM_Validation
================================
Compares PPM baseline settings against PPM_VAL_constraints.txt to validate
that settings meet defined acceptance criteria (pass/fail validation).
Re-extracts ppm_settings internally so it is fully self-contained.

PKL: <etl_basename>_df_PPM_Validation.pkl  (same folder as ETL)
Cache: skips re-analysis if PKL already exists.

Usage:
    speed.exe run standalone_df_PPM_Validation.py --etl_file <path>
                                                  [--ppm_val_constraints <file>]

PKL keys:
    df_PPM_Validation — columns: ppm_setting, actual_value, expected_value,
...
PKL_SUFFIX = "df_PPM_Validation"
_CURRENT_DIR = Path(__file__).parent.parent
DEFAULT_CONSTRAINTS = str(_CURRENT_DIR / "constraints" / "PPM_VAL_constraints.txt")
...
def analyze_validation(df_settings, constraints_file):
    expected = {}
    ...
    mismatches = (df["status"] == "MISMATCH").sum() if not df.empty else 0
    print(f"[df_PPM_Validation] {len(df)} rows, {mismatches} mismatches")
    return df

def main():
    ap = argparse.ArgumentParser(description="Standalone df_PPM_Validation (speed.exe)")
    ap.add_argument("--etl_file", required=True)
    ap.add_argument("--ppm_val_constraints", default=DEFAULT_CONSTRAINTS)
    args = ap.parse_args()
    ...
    df_validation = analyze_validation(df_settings, args.ppm_val_constraints)
    results = {
        "df_PPM_Validation": df_validation,
        "df_ppm_settings":   df_settings,
        "meta": {"analysis": PKL_SUFFIX, "etl_file": args.etl_file,
                 "constraints_file": args.ppm_val_constraints,
