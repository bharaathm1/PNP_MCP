"""
Standalone: df_c0_intervals
=============================
Extracts C0 (active / not idle) CPU intervals using trace.get_c0_intervals().
C0 intervals indicate the CPU is in the active, non-idle power state.

PKL: <etl_basename>_df_c0_intervals.pkl  (same folder as ETL)
Cache: skips re-analysis if PKL already exists.

Usage:
    speed.exe run standalone_df_c0_intervals.py --etl_file <path>

PKL keys:
    df_c0_intervals — per-core C0 active intervals (columns depend on SPEED version)
"""
...
def extract(trace):
    try:
        if not hasattr(trace, "get_c0_intervals"):
            print("[WARNING] get_c0_intervals not available"); return pd.DataFrame()
        data = trace.get_c0_intervals()
        ...
        print(f"[df_c0_intervals] {df.shape}")
        return df
    except Exception as e:
        print(f"[WARNING] extract error: {e}"); return pd.DataFrame()

def main():
    ap = argparse.ArgumentParser(description="Standalone df_c0_intervals (speed.exe)")
